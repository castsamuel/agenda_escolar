...
export function render(root) {
  ...
  root.innerHTML = `
    <div class="page-head">
      <h1>Notas</h1>
      <p class="page-sub">Peso das avaliações: AV1 ×3, AV2 ×5, AV3 ×2. Clique numa nota para editá-la.</p>
    </div>
    ...
    <div class="table-scroll">
      <table class="data-table grades-table" ${subjects.length ? '' : 'hidden'}>
        <thead><tr><th>Matéria</th><th>AV1 <small>(×3)</small></th><th>AV2 <small>(×5)</small></th><th>AV3 <small>(×2)</small></th><th>Média</th><th></th></tr></thead>
  ...
  root.querySelectorAll('.grade-input').forEach((el) => {
    el.addEventListener('change', () => {
      ...
      if (val !== null && (val < 0 || val > 10)) { toast('A nota deve estar entre 0 e 10.', 'error'); render(root); return; }
      db.setGrade(el.dataset.subject, activeBimestre, el.dataset.field, val);
      toast('Nota salva.');
      render(root);
...
function subjectRow(s, bimestre, minGrade) {
  ...
  return `<tr>
    ...
    <td><input class="grade-input" type="number" step="0.1" min="0" max="10" data-subject="${s.id}" data-field="av1" value="${row.av1 ?? ''}" /></td>
    <td><input class="grade-input" type="number" step="0.1" min="0" max="10" data-subject="${s.id}" data-field="av2" value="${row.av2 ?? ''}" /></td>
    <td><input class="grade-input" type="number" step="0.1" min="0" max="10" data-subject="${s.id}" data-field="av3" value="${row.av3 ?? ''}" /></td>
